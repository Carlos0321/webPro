...
//Math.ceil(올릴 실수값) = double 값을 리턴 
//Math.round(반올림할 실수값) = long,int (정수값)을 리턴 
//Math.floor(내릴 실수값) = double값 리턴 
public class Ex02_round {
	public static void main(String[] args) {
		System.out.println("소수점에서 반올림, 올림, 버림");
		System.out.println("9.15를 올림 : "+(int)Math.ceil(9.15));
		System.out.println("반올림" + Math.round(9.15));
		System.out.println("9.15를 내림 : "+ (int)Math.floor(9.15));
		System.out.println("소수점 첫째자리에서 반올림, 올림, 버림");
		System.out.println("9.15올림"+Math.ceil(9.15*10)/10);
		System.out.println("9.15를 반올림" + Math.round(9.15*10)/10.0); // 리턴타입이 정수라서 10.0으로 나눠야함
