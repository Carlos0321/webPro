...
public class FileboardDao {
	...
	public int getFileboardCnt() {
		...
		try {
			...
			rs=pstmt.executeQuery();
			rs.next();
			totCnt = rs.getInt("totcnt"); //엘리아스 그대로
		} catch (SQLException e) {
			System.out.println(e.getMessage());
		}finally {
			try {
	...
	private void preReplyStepA(int fref,int fre_step) {
		...
		try {
			...
			System.out.println(result == 0 ?"첫 답변" :result+"행 step 조정");
		}catch (SQLException e) {
			System.out.println(e.getMessage());
		}finally {
			try {
	...
	public int reply(FileboardDto dto) {
		...
		try {
			...
			System.out.println(e.getMessage() +"답변글쓰기 실패:"+dto);
		}finally {
			try {
				if(pstmt!=null)pstmt.close();
				if(conn!=null)conn.close();
		...
		try {
			...
			if(rs.next()) {
					// int fnum = rs.getInt("fnum");
					 ...
					 String cname =rs.getString("cname");
					 String cemail = rs.getString("cemail");
					 dto = new FileboardDto(fnum, cid, fsubject, fcontent, ffilename, fpw, fhit, fref,
	...
	public int updateBoard(FileboardDto dto) {
		...
		try {
			...
			result =pstmt.executeUpdate();
			System.out.println(result==SUCCESS ? "글수정 성공" : "해당 글 번호가 없어서 수정 실패");
		}catch (SQLException e) {
