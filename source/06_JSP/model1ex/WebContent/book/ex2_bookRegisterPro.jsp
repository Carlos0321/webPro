...
<body>
	<%//한글 인코딩 처리 - multipartRequest객체 생성 - 첨부된 파일명 get - 그 외 파라미터에서 입력된 정보 get -insert
	...
	for(String img : image){
		if(img!=null){
			...
			try{
				...
				os = new FileOutputStream("D:/Carlos/webPro/source/06_JSP/ch14_fileUp/WebContent/bookImg/"+img);
	...
	<h3>책이미지 : <%=path %>/<%=bimage2 %></h3>
	<img src="<%=conPath %>/bookImg/<%=bimage2 %>">
	<h3>책 설명</h3>
	<pre><%=bcontent %></pre>
	<h3>책 가격 : <del><%=bprice %></del><%=bprice*(100-bdiscount)/100 %>원</h3>
	<h3>할인율 : <%=bdiscount %></h3>
	<a href ="ex3_list_board.jsp">책전체리스트(게시판 스타일)</a>
	<a href ="ex3_list_boardp.jsp">책전체리스트(paging 게시판 스타일)</a>
	<a href ="ex3_list_product.jsp">책 전체리스트(쇼핑몰 스타일)</a>
