...
SELECT  ENAME, JOB,HIREDATE FROM EMP
    WHERE HIREDATE BETWEEN TO_DATE('81/02/20', 'RR/MM/DD') 
        AND TO_DATE('81/05/01', 'RR/MM/DD')   ORDER BY HIREDATE;
...
SELECT E.ENAME, E.SAL, D.DEPTNO, DNAME, LOC, NVL(M.ENAME,'없음')"M.ENAME" FROM EMP E, EMP M, DEPT D WHERE E.DEPTNO = D.DEPTNO AND E.MGR = M.EMPNO(+);
...
--6. EMP 테이블에서 직책별 급여 평균이 3000이상인 직책에 대하여 직책명, 평균급여, 금여의 합을 출력하는 SQL문을 작성하시오(5점).
...
SELECT * FROM EMP;
INSERT INTO EMP VALUES(8888,'KIM','STUDENT',NULL,SYSDATE,NULL,NULL,20);
UPDATE EMP SET JOB = 'IT' WHERE ENAME = 'KIM';
...
CREATE TABLE IDX_EMP_ENAME AS SELECT * FROM EMP;
