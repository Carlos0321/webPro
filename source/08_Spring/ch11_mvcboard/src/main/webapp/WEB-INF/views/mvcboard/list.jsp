...
<body>
	...
	<c:if test="${deleteResult eq SUCCESS}">
		<script type="text/javascript">
			alert('${param.bid} 글 삭제 성공')
		</script>
	</c:if>
	<c:if test="${deleteResult eq FAIL}">
		<script type="text/javascript">
			alert('${param.bid} 글 삭제 실패')
		</script>
	</c:if>
	
	<c:if test="${replyResult eq SUCCESS}">
		<script type="text/javascript">
			alert('${param.bid} 글 답변 성공')
		</script>
	</c:if>
	<c:if test="${replyResult eq FAIL}">
		<script type="text/javascript">
			alert('${param.bid} 글 답변 실패')
		</script>
	</c:if>
			...
			<c:forEach items="${boardList }" var="bDto">
				<tr onclick="trclicked(${bDto.bid})">
					...
					<td><fmt:formatDate value="${bDto.bdate }"
							pattern="yy/mm/dd(E) hh:mm:ss)(a)" /></td>
					<td><fmt:formatNumber value="${bDto.bhit }"
	...
	<div class="paging">
		<c:if test="${startPage > BLOCKSIZE }">
			<a href="${conPath }/mvcboard/list.do?pageNum=${startPage - 1}">
				이전 </a>
		</c:if>
